...
func restoreSystemDNS() error {
	...
	case link != "":
		...
		if err := os.Symlink(link, "/etc/resolv.conf"); err != nil {
			return fmt.Errorf("恢复 resolv.conf 符号链接: %w", err)
		}
	case marker(backup):
		_ = os.Remove("/etc/resolv.conf")
		if err := copyFile(backup, "/etc/resolv.conf", 0644); err != nil {
			return fmt.Errorf("恢复 resolv.conf: %w", err)
		}
	default:
		return fmt.Errorf("未找到安装前系统 DNS 备份，已保留当前 /etc/resolv.conf")
	}

	if marker(filepath.Join(d, "resolved.enabled")) {
		runSystemctl("enable", "systemd-resolved.service")
	}
	if marker(filepath.Join(d, "resolved.active")) {
		runSystemctl("start", "systemd-resolved.service")
	}
	return nil
}
...
func restoreOrRemoveSmartDNS(cfg Config) error {
	...
	runSystemctl("disable", "--now", "smartdns.service")

	if preexisting {
		if p := readTrim(filepath.Join(d, "binary.path")); p != "" && marker(filepath.Join(d, "smartdns.bin")) {
			if err := copyFile(filepath.Join(d, "smartdns.bin"), p, 0755); err != nil {
				return fmt.Errorf("恢复原 SmartDNS 二进制: %w", err)
			}
		}
		if marker(filepath.Join(d, "config.existed")) && marker(filepath.Join(d, "smartdns.conf")) {
			if err := copyFile(filepath.Join(d, "smartdns.conf"), cfg.SmartDNSConf, 0640); err != nil {
				return fmt.Errorf("恢复原 SmartDNS 配置: %w", err)
			}
		...
		for _, pair := range [][2]string{
			{filepath.Join(d, "default.smartdns"), "/etc/default/smartdns"},
			{filepath.Join(d, "init.smartdns"), "/etc/init.d/smartdns"},
		} {
			if marker(pair[0]) {
				mode := os.FileMode(0640)
				if strings.Contains(pair[1], "/init.d/") {
					mode = 0755
				}
		...
		if src := filepath.Join(d, "smartdns.service"); marker(src) {
			dst := readTrim(filepath.Join(d, "service.path"))
			if dst != "" {
				_ = copyFile(src, dst, 0644)
			}
		}
		runSystemctl("daemon-reload")
		if marker(filepath.Join(d, "service.enabled")) {
			runSystemctl("enable", "smartdns.service")
		}
		if marker(filepath.Join(d, "service.active")) {
			runSystemctl("start", "smartdns.service")
		}
		return nil
	}

	if installedByUs {
		if p := readTrim(filepath.Join(d, "binary.path")); p != "" {
			_ = os.Remove(p)
		} else {
			_ = os.Remove("/usr/sbin/smartdns")
		}
		_ = os.Remove("/etc/default/smartdns")
		_ = os.Remove("/etc/init.d/smartdns")
		if p := readTrim(filepath.Join(d, "service.path")); p != "" {
			_ = os.Remove(p)
		}
		_ = os.Remove("/lib/systemd/system/smartdns.service")
		_ = os.Remove("/usr/lib/systemd/system/smartdns.service")
		if b, err := os.ReadFile(cfg.SmartDNSConf); err == nil && strings.HasPrefix(string(b), "# Managed by smartunlock") {
			_ = os.Remove(cfg.SmartDNSConf)
		}
		runSystemctl("daemon-reload")
	}
...
func Uninstall(cfg Config) []error {
	...
	if err := restoreOrRemoveSmartDNS(cfg); err != nil {
		warnings = append(warnings, err)
	}
